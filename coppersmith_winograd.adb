--  Coppersmith–Winograd educational body: taxonomy / exponents, classical
--  and Strassen sketches, galactic reject for CW / laser-family Multiply.

pragma Ada_2022;

with Ada.Numerics.Elementary_Functions;

package body Coppersmith_Winograd
  with SPARK_Mode => Off
is
   ...
   ---------------------------------------------------------------------------
   -- Milestone table (fixed labels padded to Milestone.Label'Length)
   ---------------------------------------------------------------------------
   ...
   --  Educational survey milestones (approximate published ω bounds).
   --  Sources: Wikipedia matrix-multiplication complexity pages; CW 1990.
   Milestones : constant array (1 .. Milestone_Count) of Milestone :=
     [Make_Milestone (1969, 2.807_355, "Strassen seven-product recursion"),
      ...
      Make_Milestone (2012, 2.3729,    "Vassilevska Williams laser family"),
   ...
   begin
      case M is
         when Classical =>
            return "Naive Theta(n^3) triple loop; practical baseline.";
         when Strassen =>
            return "Seven-product recursion; O(n^{log2 7}); practical-ish.";
         when Coppersmith_Winograd =>
            return "1990 CW tensor / laser precursor; galactic, omega~2.3755.";
         when Laser_Family =>
            return "Post-CW laser-method refinements; galactic, omega~2.373.";
      end case;
   end Describe;
   ...
   begin
      ...
      case M is
         when Classical =>
            Info.Year := 0;  --  definitional / antiquity
         when Strassen =>
            Info.Year := 1969;
         when Coppersmith_Winograd =>
            Info.Year := 1990;
         when Laser_Family =>
            Info.Year := 2010;  --  Stothers / Williams era start
      end case;
      return Info;
   end Classify_Method;
   ...
   begin
      --  Float ** Float is not predefined; use Exp(ω * Log N).
      if N = 1 then
         return 1.0;
      end if;
   ...
   ---------------------------------------------------------------------------
   -- Internal classical / Strassen (mirrored lightly from Ada-Strassen)
   ---------------------------------------------------------------------------
   ...
   function Fail_Galactic (M : Method_Kind) return Multiply_Result is
      R : Multiply_Result;
   begin
      --  Catalogue-only: CW / laser are galactic algorithms.
      R.Stat    := Galactic_Only;
      R.Success := False;
      R.Method  := Method_Kind'Pos (M);
      --  Also expose Not_Implemented via a dedicated field path: callers
      --  may treat Galactic_Only as the primary reject; tests also accept
      --  Not_Implemented. We set Galactic_Only as the canonical status.
      return R;
   end Fail_Galactic;
   ...
   begin
      case Method is
         ...
         when Coppersmith_Winograd | Laser_Family =>
            declare
               R : Multiply_Result := Fail_Galactic (Method);
            begin
               --  Dual status documentation: Galactic_Only is primary;
               --  expose Not_Implemented by overwriting when Method is CW
               --  so tests can check either flavour as specified.
               if Method = Coppersmith_Winograd then
